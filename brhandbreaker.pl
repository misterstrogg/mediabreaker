...
#STROGG 2012
#Hastily adapted this to process bluray mkvs in a folder
...
$compressargs = $compressargs . " -q 20 ";         # Video bitrate (atv=2500,iphone=1000)
...
die "No mkv files found in $infolder\n" if( @allvidts == 0 );
...

   {
	...
	my $subtitleargs = join(',', @subtitleargsarr);
      ...
      if( $durations[$iTitle] > $timeThreshold )
      {
         ...
         if( $outfolder eq "" )
         {
	die "Cant put them nowhere, specify output folder\n";
		...
		$outname = $slash_split[-1];
		print $outname ."\n";
		$outname =~ s/_t[0-9][0-9]\.mkv\z//;
		$outname =~ s/\.mkv\z//;
		$outname =~ s/_/ /g;
		if ($title < 2) {
            $outfile = $outfolder . "\/" . $outname . "$ext";
        	} else {    
	$outfile = $outfolder . "\/" . $outname . ".title$title" . "$ext";
		}
         }
...
{
#print out the command running.
   print( $hbcmd."\n" );
#Do the actual work. (Comment this line out when testing)
   system( $hbcmd );
}
